...
/**
 * Proxy a request to the origin server (or static assets when no origin is configured).
 *
 * Priority:
 * 1. ORIGIN_SERVICE — Service Binding to another Worker
 * 2. ORIGIN_URL — External origin; URL rewrite
 * 3. ASSETS — Workers Static Assets (`public/`), for local dev without DNS origin
 * 4. fetch(request) — DNS-based origin (production zone)
 */
